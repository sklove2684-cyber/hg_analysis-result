...
- DIBK 다중 Peak는 합치지 않고 모두 보존하며, 각 Peak를 가까운 STD RT 슬롯에 배정한 뒤 슬롯별 최근접 Peak만 입력합니다.
...
- 두 경로가 모두 실패하면 Windows 사용자 바탕화면의 `분석프로그램` 폴더로 자동 전환합니다.
- 연결 상태를 다시 확인할 때 회사 공유폴더가 복구되면 공유폴더 저장 모드로 자동 복귀합니다.
...
- 작업자 Sample명은 `168`, `168-`, `168-사업장명` 형식을 같은 분석번호로 인식합니다.
- 알콜2의 IBA/n-BTOH 중복 Peak는 선택 STD 세트의 물질별 중앙 RT에 가장 가까운 Peak를 입력합니다.
- DIBK는 모든 피크를 DB에 보존하고, 선택 STD의 두 RT 슬롯별 최근접 Peak를 Excel의 Z/AA 입력 셀에 매핑합니다.
